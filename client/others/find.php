...
define ('DBUSER', 'root');
define ('DBPASS','');
define ('DBNAME','book_share_community');
...
if (isset($_REQUEST['query'])) {
    ...
    $sql = mysql_query ("SELECT * FROM book_list WHERE book_name LIKE '%{$query}%'");
	$array = array();
    while ($row = mysql_fetch_array($sql)) {
        $array[] = array (
            'label' => $row['book_name'].', '.$row['book_name'],'value' => $row['book_name'],);
